...
// Quirk flags and delays
#define XHCI_QUIRK_NEEDS_WPR   (1<<0)
#define XHCI_QUIRK_SKIP_WPR    (1<<1)
#define XHCI_QUIRK_LONG_DELAY  (1<<2)

#define XHCI_DELAY_POWER_MS    100
#define XHCI_DELAY_WPR_MS      50


// Forward declarations
static int  xhci_should_wpr(struct usb_xhci_s *xhci, u32 port, u32 portsc);
static void xhci_clear_port_changes(struct usb_xhci_s *xhci, u32 port, u32 portsc);
...
xhci_hub_reset(struct usbhub_s *hub, u32 port)
{
    ...
    case PLS_U0:
    {
        // Conditional warm reset only if heuristics/quirks say so
        if (xhci_should_wpr(xhci, port, portsc)) {
            dprintf(3, "%s: port %u issuing WPR\n", __func__, port + 1);
            xhci_clear_port_changes(xhci, port, portsc);
            writel(&xhci->pr[port].portsc, portsc | XHCI_PORTSC_WPR);
            msleep(XHCI_DELAY_WPR_MS);
        } else {
            dprintf(3, "%s: port %u skipping WPR\n", __func__, port + 1);
        }
        break;
    }
    case PLS_POLLING:
...
configure_xhci(void *data)
{
    ...
    /* Power on ports with tunable delay and log initial state */
    if (xhci->ports) {
        for (u32 i = 0; i < xhci->ports; i++) {
            u32 portsc = readl(&xhci->pr[i].portsc);
            if (!(portsc & XHCI_PORTSC_PP)) {
                writel(&xhci->pr[i].portsc, portsc | XHCI_PORTSC_PP);
            }
            xhci_print_port_state(3, "XHCI power", i, readl(&xhci->pr[i].portsc));
        }
        msleep(XHCI_DELAY_POWER_MS);
    }

    /* Enable interrupter 0 and master interrupt */
    {
        u32 iman = readl(&xhci->ir->iman);
        writel(&xhci->ir->iman, iman | 0x1);
        u32 usbcmd = readl(&xhci->op->usbcmd);
        writel(&xhci->op->usbcmd, usbcmd | XHCI_CMD_INTE);
        dprintf(3, "XHCI interrupts enabled: IMAN=0x%x, USBCMD=0x%x\n",
                readl(&xhci->ir->iman), readl(&xhci->op->usbcmd));
    }
...
static void xhci_process_events(struct usb_xhci_s *xhci)
{
    ...
    for (;;) {
        ...
        case ER_PORT_STATUS_CHANGE:
        {
            ...
            // Clear RW1C bits robustly and re-log state
            xhci_clear_port_changes(xhci, port, portsc);
            u32 after = readl(&xhci->pr[port].portsc);
            xhci_print_port_state(3, __func__, port, after);
            break;
        }
        default:
               ...
               , void *data, int datalen)
{
    ...
    /* Accept short packets as valid (needed for HID enumeration) */
...
static u32
xhci_get_quirks(struct usb_xhci_s *xhci)
{
    u32 quirks = 0;

    // Default: conservative delays
    quirks |= XHCI_QUIRK_LONG_DELAY;

    // Use PCI vendor/device if present
    if (xhci->usb.pci) {
        u16 ven = xhci->usb.pci->vendor;
        u16 dev = xhci->usb.pci->device;

        // Examples — adjust as you learn real devices in testing:
        // Intel desktop (older gen): often sensitive to WPR
        if (ven == 0x8086) {
            // Tentatively skip WPR on older Intel desktop parts
            quirks |= XHCI_QUIRK_SKIP_WPR;
        }

        // ASMedia controllers typically need longer delays, WPR sometimes helpful
        if (ven == 0x1b21) {
            quirks &= ~XHCI_QUIRK_SKIP_WPR;
            quirks |= XHCI_QUIRK_NEEDS_WPR | XHCI_QUIRK_LONG_DELAY;
        }

        // You can add precise (ven,dev) cases here as reports come in.
        (void)dev;
    }
...
static int
xhci_should_wpr(struct usb_xhci_s *xhci, u32 port, u32 portsc)
{
    u32 quirks = xhci_get_quirks(xhci);

    if (quirks & XHCI_QUIRK_SKIP_WPR)
        return 0;
    if (quirks & XHCI_QUIRK_NEEDS_WPR)
        return 1;

    // Heuristic: only consider WPR for SuperSpeed ports stuck in U0 without PED
    u32 speed = xhci_get_field(portsc, XHCI_PORTSC_SPEED);
    u32 pls   = xhci_get_field(portsc, XHCI_PORTSC_PLS);
    if (speed == 4 && pls == PLS_U0 && !(portsc & XHCI_PORTSC_PED))
        return 1;
...
static void
xhci_clear_port_changes(struct usb_xhci_s *xhci, u32 port, u32 portsc)
{
    // Clear RW1C bits individually, re-reading between clears for robustness
    const u32 bits[] = {
    ...
    for (int i = 0; i < (int)(sizeof(bits)/sizeof(bits[0])); i++) {
        u32 cur = readl(&xhci->pr[port].portsc);
        if (cur & bits[i]) {
            writel(&xhci->pr[port].portsc, cur | bits[i]);
        }
    }
}