...
// --- New: ARGB text helpers from cbvga.c ---
extern void cbvga_init_text_palette(void);
extern void cbvga_render_char(u16 cx, u16 cy, u8 ch, u8 attr);
extern void cbvga_clear_region(u16 x, u16 y, u16 cols, u16 rows, u8 attr);

// --- New: cursor helpers ---
extern void cbvga_draw_cursor(u16 cx, u16 cy, u8 attr, int start, int end);
extern void cbvga_hide_cursor(u16 cx, u16 cy, u8 attr);
                  ...
                  , struct carattr ca)
{
    ...
    // --- New: also clear ARGB framebuffer region ---
    cbvga_clear_region(win.x, win.y, winsize.x, winsize.y, ca.attr);
}

// Scroll characters within a window on the screen
void vgafb_scroll(struct cursorpos win,
                  struct cursorpos winsize,
                  int lines, struct carattr ca)
{
    ...
    if (lines > 0) {
        ...
        struct cursorpos clearpos = { win.x, win.y + winsize.y - lines, 0, 0 };
        struct cursorpos clearsize = { winsize.x, lines, 0, 0 };
        vgafb_clear_chars(clearpos, clearsize, ca);
    } else {
        // scroll down
        int nlines = -lines;
        vgafb_move_chars(win, winsize, -nlines);
        struct cursorpos clearpos = { win.x, win.y, 0, 0 };
        struct cursorpos clearsize = { winsize.x, nlines, 0, 0 };
        vgafb_clear_chars(clearpos, clearsize, ca);
    }

    // --- New: mirror into ARGB framebuffer ---
    if (GET_GLOBAL(curmode_g->memmodel) == MM_TEXT) {
        if (!lines) {
            cbvga_clear_region(win.x, win.y, winsize.x, winsize.y, ca.attr);
        } else if (lines > 0) {
            for (int row = 0; row < winsize.y - lines; row++) {
                for (int col = 0; col < winsize.x; col++) {
                    struct carattr c = vgafb_read_char(
                        (struct cursorpos){win.x + col, win.y + row, 0, 0});
                    cbvga_render_char(win.x + col, win.y + row, c.car, c.attr);
                }
            }
            cbvga_clear_region(win.x, win.y + winsize.y - lines,
                               winsize.x, lines, ca.attr);
        } else {
            int nlines = -lines;
            for (int row = winsize.y - 1; row >= nlines; row--) {
                for (int col = 0; col < winsize.x; col++) {
                    struct carattr c = vgafb_read_char(
                        (struct cursorpos){win.x + col,
                                           win.y + row - nlines, 0, 0});
                    cbvga_render_char(win.x + col, win.y + row, c.car, c.attr);
                }
            }
            cbvga_clear_region(win.x, win.y, winsize.x, nlines, ca.attr);
        }
    }
}
...
void vgafb_write_char(struct cursorpos cp, struct carattr ca) {
    ...
    // --- New: also render into ARGB framebuffer ---
    cbvga_render_char(cp.x, cp.y, ca.car, ca.attr);
}
