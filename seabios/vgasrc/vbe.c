...
// -----------------------------------------------------------------------------
// Keep a global copy of the last VBE mode info for cbvga emulator
// -----------------------------------------------------------------------------
struct vbe_mode_info g_last_vbe_mode_info VAR16;

struct vbe_mode_info *get_current_vbe_mode_info(void) {
    return &g_last_vbe_mode_info;
}
...
vbe_104f01(struct bregs *regs)
{
    ...
    // --- 4bpp override to make the mode valid and selectable ---
    if (depth == 4 && memmodel == MM_PACKED) {
        // Packed 4bpp stride: ceil(width/2) aligned to 4 bytes
        int packed_pitch = (((width + 1) / 2) + 3) & ~3;

        // Pitch fields for banked framebuffer
        SET_FARVAR(seg, info->bytes_per_scanline, packed_pitch);

        // Do not claim linear 4bpp framebuffer on GOP hardware
        SET_FARVAR(seg, info->phys_base, 0);
        SET_FARVAR(seg, info->linear_bytes_per_scanline, 0);
        SET_FARVAR(seg, info->linear_pages, 0);

        // Planes/model and attributes consistent with indexed graphics
        SET_FARVAR(seg, info->planes, 1);
        SET_FARVAR(seg, info->mem_model, MM_PACKED);

        u16 idx_mode_attr = VBE_MODE_ATTRIBUTE_SUPPORTED |
                            VBE_MODE_ATTRIBUTE_COLOR_MODE |
                            VBE_MODE_ATTRIBUTE_GRAPHICS_MODE;
        SET_FARVAR(seg, info->mode_attributes, idx_mode_attr);

        // Indexed color → no direct masks
        SET_FARVAR(seg, info->red_size,   0);
        SET_FARVAR(seg, info->red_pos,    0);
        SET_FARVAR(seg, info->green_size, 0);
        SET_FARVAR(seg, info->green_pos,  0);
        SET_FARVAR(seg, info->blue_size,  0);
        SET_FARVAR(seg, info->blue_pos,   0);
        SET_FARVAR(seg, info->alpha_size, 0);
        SET_FARVAR(seg, info->alpha_pos,  0);

        // Bank/page fields kept simple
        SET_FARVAR(seg, info->banks, 1);
        SET_FARVAR(seg, info->pages, 0);
    }

    // --- NEW: copy to global for emulator ---
    memcpy_far(get_global_seg(), &g_last_vbe_mode_info,
               seg, info, sizeof(struct vbe_mode_info));
